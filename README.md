...
- **Art:** block and item textures, GUI icons (`textures/gui/sprites/icon/*.png`, 9×9) are generated by `tools/make_art.py` (the pixel maps are in the script) and the mod icon by `tools/make_icon.py` (32×32, drawn in code; `pip install pillow`). The interface draw language lives in `Ui.java` (embossed panels and buttons, sunken fields, icons).
