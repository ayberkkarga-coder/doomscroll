...
- **Oylamalı video sırası:** sıra artık sunucuda duruyor, yani herkes aynı listeyi görüyor ve herkes ekleyebiliyor. `/ds sira ekle <adres>`, kumandadaki **+** ya da tabletteki **Sıraya**. Oynayan video bitince **en çok oyu alan** açılır ve herkes aynı adrese geçer. Karttaki sayıya basınca oy verir ya da geri alırsın; bir şey eklemek ona oy vermek sayılır, zaten sırada olan bir adresi eklemek de yalnızca oy verir. `/ds sira` oy sayılarıyla listeler, `/ds sira oyla <no>`, `/ds sira atla`, `/ds sira sil <no>` (kendi eklediğin, ekran senin ise hepsi), `/ds sira temizle` (ekran sahibi). En fazla 50 video, oturumluk.
...
- **YouTube reklamlarına dokunulmuyor.** Videoyla aynı alan adından geldikleri için filtre listeleri yetişemiyor; mod da atlamıyor. Tarayıcıda nasıl görüyorsan öyle.
...
- **Tasarım:** blok/eşya dokuları, GUI simgeleri (`textures/gui/sprites/icon/*.png`, 9×9) `tools/make_art.py` ile (piksel haritaları betiğin içinde), mod simgesi `tools/make_icon.py` ile üretilir (32×32, kodla çizilir; `pip install pillow`). Arayüz çizim dili `Ui.java` (kabartma panel/tuş, çukur alan, simge).
